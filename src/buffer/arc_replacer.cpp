...
void ArcReplacer::RecordAccess(frame_id_t frame_id, page_id_t page_id, [[maybe_unused]] AccessType access_type) {
  ...
  if (it_ghost != ghost_map_.end()) {
    ...
    if (it_ghost != ghost_map_.end()) {
      ...
      // FIX: Use delta logic for adaptation
      if (was_mru_ghost) {
        // Hit in B1 (MRU Ghost): Increase p
        size_t b1_size = mru_ghost_.size();
        size_t b2_size = mfu_ghost_.size();
        size_t delta = (b1_size >= b2_size) ? 1 : b2_size / b1_size;
        mru_target_size_ = std::min(mru_target_size_ + delta, replacer_size_);
      } else {
        // Hit in B2 (MFU Ghost): Decrease p
        size_t b1_size = mru_ghost_.size();
        size_t b2_size = mfu_ghost_.size();
        size_t delta = (b2_size >= b1_size) ? 1 : b1_size / b2_size;

        // Use saturation arithmetic to prevent underflow
